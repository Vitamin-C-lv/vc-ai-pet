...
用户首轮实测“花花在吗”三次才有反应。日志显示“花花在吗”被开放识别成“花花在忙”，旧策略曾把“在忙”错误转成提问。随后用户报告再叫三次仍无可感知反应；Bridge 日志中一次 VAD 候选实际被接受为 `wake-only`，但设备没有听觉确认，聆听状态也未传给实体屏幕；另一次开放识别“花在了”被拒。现已部署 Bridge-only 修正：限定语法与开放识别共同确认时接受该实测变体；wake-only 排入本地“我在。”语音确认、向屏幕传 `listening`，扬声器 ACK 后重新开始 9 秒追问窗口。单测通过，Bridge 服务 active、设备持续轮询；等待用户再次实机验收，不能声称唤醒体验已最终通过。Pet `/api/pet/state` 可返回 200；Bridge event feed 曾短时报告 `fetch failed`，需继续观察。公开词模型请求未提交。

用户复测报告“两次才有反应”：首次 VAD 开放识别为“哈哈在吗”（置信度 0.714），限定词识别为“花花在吗”（置信度 1），旧规则拒绝；第二次设备 `huahua` 候选被接受为 `wake-only`，实体扬声器 ACK `ok:true`。Bridge 已增加仅对完整短句“哈哈在吗”、限定词为“花花在吗”且双路置信度分别不低于 0.7/0.95 的受限识别；没有降低通用阈值或接受任意“哈哈”语音。单测通过；仍待新规则部署后的一次命中及误唤醒实测。

新规则部署后的带时间标记实测：用户一次说“花花在吗”即听到“我在”；Bridge 10:58:59 UTC 收到 VAD 全量及限定词均为“花花在吗”的 `wake-only`，10:59:03 收到实体扬声器 ACK `ok:true`，首次唤醒成功。随后在聆听窗口内背景片段被 Vosk 听成单字“那”（置信度 0.466），误当追问提交；已加追问清晰度门槛（至少 2 字、置信度不低于 0.6），单测覆盖误提交与正常短问句。仍需复测真实追问和无误唤醒，不能把整体体验标为最终通过。

追问验收下一轮首次未唤醒：设备 `huahua` 候选分数 0.801，限定词为“花花在吗”，开放识别为“画画在吗”（整体置信度 0.721），旧规则判 `full-transcript-mismatch`。已将该完整短句纳入双路确认的受限变体（限定词置信度至少 0.95、开放识别至少 0.7），普通“画画”和其他句子仍不接受；等待部署后再次实机验证。

最终受限变体部署后的实机闭环：用户确认一次说“花花在吗”即听到“我在”，再说“你好吗”得到实体扬声器回答。Bridge 对应 11:02:23 UTC 设备候选经本机双路识别接受 `wake-only`（开放识别“哈哈在吗”0.706、限定词“花花在吗”1），11:02:27 speaker ACK `ok:true`；11:02:30 追问准确识别为“你好吗”（置信度 1）并提交；11:02:48 回复 speaker ACK `ok:true`。普通话负例仍待实机反馈；不将单次正例夸大为长期命中率证明。

用户对普通话负例回复“没有错误”，未报告误唤醒。后续只读核对还发现不清晰追问分支没有主动检查 9 秒超时：仅在健康检查或后续有效语音触发 `tick`，聆听状态可能滞留。已让每次追问判定先按时钟刷新会话，并加入超时单测。另在 11:50:04 UTC 见一次双路确认为“花花在吗”且实体播报 ACK `ok:true`，无法仅凭后台日志判定是否用户主动呼叫；不可计入负例结果。

用户随后确认 11:50 UTC 左右确实主动叫过“花花在吗”，该次不是误唤醒。当前生产 Bridge 运行最新超时修正，实体持续连接；已实测一次唤醒、追问、回复完整闭环及一条普通话负例，无已确认误唤醒。单次验收证明当前直接可用，不等同于长期唤醒率统计；如以后出现连续漏唤醒，应重新按同一时间窗口收集候选与识别证据。

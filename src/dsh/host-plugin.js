...
export function apply(ctx, rawConfig = {}) {
  ...
  const runtime = new PetRuntime({
    sandboxRoot,
    logger: ctx.logger,
    // Experience-aware Memory Pipeline knobs (working window, buffer retention,
    // consolidation triggers). Omitted values fall back to the frozen defaults
    // in src/memory/memory-pipeline-config.js, so an existing profile config

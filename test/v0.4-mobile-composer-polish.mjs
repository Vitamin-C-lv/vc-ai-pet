...
assert.ok(mobileJs.includes('if (completed) {\n      clearImageSelection()'), 'image cleanup is success-only')
assert.ok(mobileJs.includes('restoreImageSelection(pendingImage)'), 'failed image sends restore the attachment')

...
const mobileCss = await readFile(join(root, 'src/remote/mobile-ui/mobile.css'), 'utf8')
assert.match(html, /type="file"/u)
assert.match(html, /accept="image\/jpeg,image\/png,image\/webp"/u)
assert.match(html, /<div id="image-preview" class="image-preview" hidden>/u)
assert.match(html, /id="image-thumbnail" alt="待发送图片预览"/u)
assert.match(html, /id="remove-image"[^>]*>×/u)
assert.match(mobileCss, /\.image-preview\[hidden\]\s*\{\s*display:\s*none;\s*\}/u)
...
assert.match(mobileJs, /imagePreview\.hidden = false/u)
assert.match(mobileJs, /imagePreview\.hidden = true/u)
assert.match(mobileJs, /imageThumbnail\.removeAttribute\('src'\)/u)

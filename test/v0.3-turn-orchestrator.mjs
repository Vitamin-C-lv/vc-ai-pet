...
assert.deepEqual(Array.from(compareEvents.events, (event) => event.type), ['turn_started', 'thinking', 'visual_selected', 'visual_image', 'visual_observation', 'visual_selected', 'visual_image', 'visual_observation', 'visual_compare', 'assistant_message', 'assistant_message', 'turn_completed'])
...
assert.equal((await store.listForRecentVisualRecall()).filter((message) => message.kind === 'final').length, 3)

...
assert.match(composerJs, /sendButton\.hidden = !canSend/u)
assert.match(composerJs, /addButton\.disabled = disabled/u)
assert.match(composerJs, /syncComposerTextareaHeight\(input\)/u)
...
assert.match(mobileJs, /openExistingImagePicker:\s*async\s*\(\)\s*=>\s*imageInput\.click\(\)/u)
